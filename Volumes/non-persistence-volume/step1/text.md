...
Create a deploy named `deploy-with-emptydir` using image `nginx` with `emptyDir` size limit of `100Mi` in the `default` namespace. Additionally, please mount the path under `/cache` with volume name `temp-volume` inside the pod
